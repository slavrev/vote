...
<html>
<head>
  <title>Регистрация проверяющего (исправить)</title>
...
%>
  ...
  Номер паспорта<br>
  <input type="text" id="id" name="id" value="<%= checker.id %>"><br>
  <br>
  ...
  Ваше ФИО <br>
  <input type="text" id="fullname" value="<%= checker.fullname %>"><br>
  <br>

  Ваш email <br>
  <input type="text" id="email" value="<%= checker.email %>"><br>
  <br>
  ...
  <input type="checkbox" id="permission" checked><label for="permission">Я разрешаю использовать мои личные данные для голосования и подсчёта голосов</label><br>
  <input type="checkbox" id="sendemails" <%= checker.sendemails ? "checked" : "" %>><label for="sendemails">Присылать мне уведомления на почту</label><br>
  <br>

  <button id="registerbutton">Отправить</button>
...
document.querySelector('#localityid').value = '<%= checker.localityid %>';
loadDistricts();
      ...
      if (this.readyState == 4) {
        ...
        if( this.status == 200 ) {
          ...
          if( firstdistrictload ) {
            ...
            districtElement.value = '<%= checker.districtid %>';
...
function registerChecker() {
  ...
  xmlhttp.onreadystatechange = function() {
      if (this.readyState == 4) {
        ...
        if( this.status == 200 ) {
          ...
          alert('Данные отправлены. Дождитесь подтверждения. Оно придёт к вам на почту.');
  ...
  xmlhttp.open("POST", "/registerchecker/fix", true);
